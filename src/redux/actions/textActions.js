...
export const createTextNode = name => async dispatch => {
  ...
  try {
    ...
    history.push('/edit/text/' + response.data.id);
  } catch (err) {
...
export const fetchTextNode = id => async dispatch => {
  dispatch({ type: FETCH_TEXT_NODE });
  try {
    const response = await instance.get('/text', { params: { id } });
    dispatch({
...
export const deleteTextNode = id => async dispatch => {
  dispatch({ type: DELETE_TEXT_NODE });
  try {
    await instance.delete(`/text`, { params: { id } });
    dispatch({ type: DELETE_TEXT_NODE_SUCCESS, id: id });
...
export const processTextNode = (id, summary) => async dispatch => {
  dispatch({ type: PROCESS_TEXT_NODE });
  try {
    await instance.patch('/text/process', { id, summary });
    dispatch({ type: PROCESS_TEXT_NODE_SUCCESS, id, summary });
  } catch (err) {
